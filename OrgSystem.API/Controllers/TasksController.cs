...
public class TasksController(AppDbContext db) : ControllerBase
{
    ...
    [HttpGet]
    public async Task<IActionResult> List([FromQuery] string? scope, [FromQuery] TaskItemStatus? status, [FromQuery] string? projectId, CancellationToken ct)
    {
        var q = db.Tasks.AsNoTracking().AsQueryable();
        var canManageProjectTasks = User.IsInRole("Admin") || User.FindAll("permission").Any(x => x.Value == "tasks.assign");
    ...
    public async Task<IActionResult> Create(TaskRequest request, CancellationToken ct)
    {
        if (request.AssignedToUserId.HasValue && request.AssignedToUserId.Value != UserId &&
            ...
            return Forbid();
        var item = new TaskItem { Title = request.Title.Trim(), Description = request.Description, ProjectId = request.ProjectId, Status = request.Status, Priority = request.Priority,
    ...
    {
        var canManageProjectTasks = User.IsInRole("Admin") || User.FindAll("permission").Any(x => x.Value == "tasks.assign");
        var item = await db.Tasks.FirstOrDefaultAsync(x => x.Id == id && (canManageProjectTasks || x.AssignedToUserId == UserId || x.AssignedByUserId == UserId), ct);
...
public record TaskRequest(string Title, string? Description, string? ProjectId, TaskItemStatus Status, TaskPriority Priority, DateTime? StartDate,
