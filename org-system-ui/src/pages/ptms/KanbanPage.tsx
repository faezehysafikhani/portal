            ...
            >
              ...
              {colTasks.map(task => (
                <Card
                  ...
                  style={{ marginBottom: 8, cursor: 'grab', borderRight: `3px solid ${getPriorityColor(task.priority) === 'red' ? '#f5222d' : getPriorityColor(task.priority) === 'orange' ? '#fa8c16' : getPriorityColor(task.priority) === 'gold' ? '#fadb14' : '#52c41a'}`, borderRadius: 8 }}
                ...
                >
                  ...
                  <div style={{ marginBottom: 6 }}>
                    ...
                    {task.tags.slice(0, 1).map(t => <Tag key={t} style={{ fontSize: 9 }}>{t}</Tag>)}
                  </div>

                  {/* پیشرفت */}
                  {task.progress > 0 && (
                    <Progress percent={task.progress} size="small" strokeColor="#8B1A6B" style={{ marginBottom: 6 }} />
                  )}
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 6 }}>
                    ...
                    {task.deadline && (
                      <span style={{ fontSize: 10, color: '#f5222d' }}>
              ...
              <Button
                type="dashed"
                icon={<PlusOutlined />}
                style={{ width: '100%', marginTop: 4, borderColor: col.color, color: col.color }}
                size="small"
              >
                افزودن
              </Button>
