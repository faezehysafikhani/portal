...
export default function SettingsPage() {
  ...
  const testSms=async()=>{setSmsTesting(true);try{if(!await saveSms(false))return;const v=await smsTestForm.validateFields();const r=await fetch(`${api}/sms-settings/test`,{method:'POST',headers:headers(),body:JSON.stringify(v)});const result=await r.json().catch(()=>({}));r.ok?message.success(result.message||'پیامک آزمایشی ارسال شد'):message.error(result.message||'ارسال ناموفق')}finally{setSmsTesting(false)}}
    ...
    },
    {
      key:'8',label:<span>📱 پنل پیامکی</span>,children:<Card title="تنظیمات پنل پیامکی">
        ...
        <Form form={smsForm} layout="vertical" initialValues={{providerName:'Kavenegar',apiUrl:'https://api.kavenegar.com/v1/',isActive:false,letterTemplate:'نامه شماره {number} مورخ {date} با موضوع «{subject}»',referralTemplate:'نامه با موضوع «{subject}» مورخ {date} با ارجاع «{referralType}»',meetingTemplate:'دعوت جلسه: {title} - {date} {time}'}}>
          ...
          <Col span={24}><Form.Item name="isActive" label="فعال‌سازی ارسال پیامک" valuePropName="checked"><Switch checkedChildren="فعال" unCheckedChildren="غیرفعال"/></Form.Item></Col>
        ...
        </Form><Divider>ارسال آزمایشی</Divider><Form form={smsTestForm} layout="inline"><Form.Item name="phone" rules={[{required:true,pattern:/^09\d{9}$/}]}><Input placeholder="09123456789" inputMode="numeric"/></Form.Item><Form.Item name="message" rules={[{required:true,max:500}]}><Input placeholder="متن آزمایشی"/></Form.Item><Button loading={smsTesting} disabled={smsTesting} onClick={()=>void testSms()}>ذخیره و ارسال تست</Button></Form>
      </Card>
    },
    ...(canManageAi ? [{
