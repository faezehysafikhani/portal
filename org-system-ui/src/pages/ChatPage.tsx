...
export default function ChatPage(){
  ...
  const closeGroup=()=>{setActiveGroup(undefined);history.replaceState(null,'',selectedId?`/chat?user=${selectedId}`:'/chat')}
  ...
  return <div style={{height:'calc(100vh - 125px)',display:'flex',gap:14,minHeight:520}}>
    ...
    <Card style={{flex:1,borderRadius:14,overflow:'hidden'}} styles={{body:{height:'100%',padding:0,display:'flex',flexDirection:'column'}}}>
      {activeGroup?<GroupConversation group={activeGroup} users={users} onClose={closeGroup} onChanged={loadGroups}/>:!selected?<Empty style={{margin:'auto'}} description="یک همکار یا گروه را انتخاب کنید"/>:<>
