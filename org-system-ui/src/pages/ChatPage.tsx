...
function GroupConversation({group,users,onChanged}:{group:ChatGroup;users:ChatUser[];onChanged:()=>Promise<void>|void}){
  ...
  return <div style={{height:'100%',display:'flex',flexDirection:'column'}}>
    ...
    <Modal open={membersOpen} onCancel={()=>setMembersOpen(false)} footer={null} centered width={520} title={<Space><TeamOutlined/><span>اعضای {group.name}</span><Tag>{group.memberCount.toLocaleString('fa-IR')} عضو</Tag></Space>}>
      <div style={{maxHeight:330,overflowY:'auto',display:'grid',gap:8,marginBottom:group.isAdmin?16:0}}>{group.members.map(member=><div key={member.userId} style={{display:'flex',alignItems:'center',gap:10,padding:'9px 10px',border:'1px solid #f0e8ee',borderRadius:10}}><Avatar size={32} icon={<UserOutlined/>}/><div style={{flex:1,minWidth:0}}><b>{member.fullName}</b><div style={{fontSize:11,color:'#888'}}>{member.position||'همکار'}{member.isAdmin?' • مدیر گروه':''}</div></div>{group.isAdmin&&member.userId!==group.ownerUserId&&<Tooltip title="حذف از گروه"><Button type="text" danger icon={<CloseOutlined/>} loading={removingMemberId===member.userId} disabled={Boolean(removingMemberId)} onClick={()=>removeMember(member)} aria-label={`حذف ${member.fullName} از گروه`}/></Tooltip>}</div>)}</div>
      {group.isAdmin&&<div style={{display:'flex',gap:8,paddingTop:14,borderTop:'1px solid #f0f0f0'}}><Select mode="multiple" value={newMemberIds} onChange={setNewMemberIds} showSearch optionFilterProp="label" maxTagCount="responsive" style={{flex:1}} placeholder="افزودن همکار جدید به گروه" options={users.filter(user=>user.personType==='user'&&!group.members.some(member=>member.userId===user.personId)).map(user=>({value:user.personId,label:user.fullName}))}/><Button icon={<PlusOutlined/>} loading={addingMembers} disabled={!newMemberIds.length} onClick={addMembers}>افزودن</Button></div>}
    </Modal>
  </div>
}
...
export default function ChatPage(){
  ...
  return <div style={{height:'calc(100vh - 125px)',display:'flex',gap:14,minHeight:520}}>
    ...
    <Card style={{flex:1,borderRadius:14,overflow:'hidden'}} styles={{body:{height:'100%',padding:0,display:'flex',flexDirection:'column'}}}>
      {activeGroup?<GroupConversation group={activeGroup} users={users} onChanged={loadGroups}/>:!selected?<Empty style={{margin:'auto'}} description="یک همکار یا گروه را انتخاب کنید"/>:<>
