...
const chatText=(value:string)=>value.split(emojiToken).map((part,index)=>emojiOnly.test(part)?<span key={`${part}-${index}`} style={{fontSize:23,lineHeight:1,verticalAlign:'middle',display:'inline-block',margin:'0 1px'}}>{part}</span>:part)
...
function GroupConversation({group,users,onClose,onChanged}:{group:ChatGroup;users:ChatUser[];onClose:()=>void;onChanged:()=>void}){
  ...
  return <div style={{height:'100%',display:'flex',flexDirection:'column'}}>
    <div style={{padding:'12px 18px',borderBottom:'1px solid #eee',display:'flex',alignItems:'center',gap:10}}>
      ...
      <div><b>{group.name}</b><div style={{fontSize:11,color:'#888'}}>{group.memberCount.toLocaleString('fa-IR')} عضو</div></div>
      <Button type="text" icon={<CloseOutlined/>} onClick={onClose} style={{marginRight:'auto'}} aria-label="بستن گفتگوی گروه"/>
    </div>
    <div style={{padding:'8px 14px',borderBottom:'1px solid #f0f0f0',maxHeight:72,overflowY:'auto'}}><Space wrap>{group.members.map(member=><Tag key={member.userId} color={member.isAdmin?'purple':'default'}>{member.fullName}{member.isAdmin?' • مدیر گروه':''}</Tag>)}</Space></div>
    {group.isAdmin&&<div style={{display:'flex',gap:8,padding:'9px 14px',borderBottom:'1px solid #f0f0f0'}}><Select mode="multiple" value={newMemberIds} onChange={setNewMemberIds} showSearch optionFilterProp="label" maxTagCount="responsive" style={{flex:1}} placeholder="افزودن همکار جدید به گروه" options={users.filter(user=>user.personType==='user'&&!group.members.some(member=>member.userId===user.personId)).map(user=>({value:user.personId,label:user.fullName}))}/><Button icon={<PlusOutlined/>} loading={addingMembers} disabled={!newMemberIds.length} onClick={addMembers}>افزودن</Button></div>}
    <div style={{flex:1,minHeight:0,overflowY:'auto',padding:20,background:'linear-gradient(145deg,#fafafa,#f7f0f5)'}}>{items.length===0?<Empty description="هنوز پیامی در گروه ثبت نشده"/>:items.map(item=><div key={item.id} style={{display:'flex',justifyContent:item.isMe?'flex-start':'flex-end',marginBottom:12}}><div style={{maxWidth:'72%'}}><div style={{fontSize:10,color:'#8b1a6b',marginBottom:2}}>{item.senderName}</div><div style={{background:item.isMe?'#8b1a6b':'#fff',color:item.isMe?'#fff':'#333',padding:'9px 14px',borderRadius:item.isMe?'14px 14px 4px 14px':'14px 14px 14px 4px',boxShadow:'0 2px 8px #0000000c',whiteSpace:'pre-wrap',overflowWrap:'anywhere'}}>{chatText(item.content)}</div><div style={{fontSize:10,color:'#999',marginTop:3}}>{new Date(item.createdAt).toLocaleString('fa-IR')}</div></div></div>)}<div ref={end}/></div>
    <div style={{padding:'10px 14px',borderTop:'1px solid #eee',display:'flex',gap:8,alignItems:'center'}}><Input.TextArea value={text} maxLength={2000} autoSize={{minRows:1,maxRows:4}} onChange={e=>setText(e.target.value)} onPressEnter={e=>{if(!e.shiftKey){e.preventDefault();send()}}} placeholder={`پیام در گروه ${group.name}...`}/><Button type="primary" icon={<SendOutlined/>} loading={sending} disabled={!text.trim()} onClick={send} style={{background:'#8b1a6b'}}>ارسال</Button></div>
  </div>
}
...
export default function ChatPage(){
  ...
  return <div style={{height:'calc(100vh - 125px)',display:'flex',gap:14,minHeight:520}}>
    ...
    <Card style={{flex:1,borderRadius:14,overflow:'hidden'}} styles={{body:{height:'100%',padding:0,display:'flex',flexDirection:'column'}}}>
      {activeGroup?<GroupConversation group={activeGroup} users={users} onClose={()=>{setActiveGroup(undefined);history.replaceState(null,'',selectedId?`/chat?user=${selectedId}`:'/chat')}} onChanged={loadGroups}/>:!selected?<Empty style={{margin:'auto'}} description="یک همکار یا گروه را انتخاب کنید"/>:<>
