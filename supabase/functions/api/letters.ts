...
export async function handleLetters(request: Request, auth: AuthContext, path: string, url: URL): Promise<Response | null> {
  ...
  if (request.method === 'GET' && path === '/letters') {
    const registry = url.searchParams.get('scope') === 'registry'; if (registry ? !canRegistry : !canInbox) throw new HttpError(403, 'دسترسی غیرمجاز')
    ...
    const result = await query.order('CreatedAt', { ascending: false }); check(result.error); const output: Obj[] = []
    for (const letter of result.data ?? []) {
      const rec = await db.from('LetterRecipients').select('*').eq('TenantId', auth.tenantId).eq('LetterId', letter.Id).eq('IsDeleted', false); check(rec.error)
      const visible = registry || canRegistry || letter.FromUserId === auth.userId || (rec.data ?? []).some((x) => x.UserId === auth.userId); if (!visible) continue
      output.push({ id: letter.Id, subject: letter.Subject, letterNumber: letter.LetterNumber, letterDate: letter.LetterDate, type: enumOut(letter.Type, types), status: enumOut(letter.Status, statuses), priority: enumOut(letter.Priority, priorities), classification: letter.Classification, fromUserName: letter.FromUserName, hasAttachment: letter.HasAttachment, toExternalName: letter.ToExternalName, toExternalOrg: letter.ToExternalOrg, incomingFromOrg: letter.IncomingFromOrg, createdAt: letter.CreatedAt, sentAt: letter.SentAt, recipientCount: rec.data?.length ?? 0, isRead: (rec.data ?? []).some((x) => x.UserId === auth.userId && x.IsRead), isSender: letter.FromUserId === auth.userId })
    }
  ...
  if (request.method === 'PUT' && match) {
    ...
    const result = await db.from('Letters').update(update).eq('TenantId', auth.tenantId).eq('Id', match[1]).select().single(); check(result.error); return json(request, { message: 'نامه ذخیره شد', letterNumber: result.data.LetterNumber, status: enumOut(result.data.Status, statuses) })
  }
