...
export async function handleReports(request: Request, auth: AuthContext, path: string): Promise<Response | null> {
  ...
  const [lettersR, tasksR, ticketsR, formsR, smsR, usersR, customersR] = await Promise.all([
    db.from('Letters').select('*').eq('TenantId', auth.tenantId).eq('IsDeleted', false).order('CreatedAt', { ascending: false }).limit(100),
    db.from('Tasks').select('*').eq('TenantId', auth.tenantId).eq('IsDeleted', false).order('CreatedAt', { ascending: false }).limit(100),
    db.from('Tickets').select('*').eq('TenantId', auth.tenantId).eq('IsDeleted', false).order('CreatedAt', { ascending: false }).limit(100),
    db.from('OrganizationalForms').select('*').eq('TenantId', auth.tenantId).eq('IsDeleted', false).order('CreatedAt', { ascending: false }).limit(100),
  ...
  ]); [lettersR, tasksR, ticketsR, formsR, smsR, usersR, customersR].forEach((x) => check(x.error))
  ...
  return json(request, {
    summary: { letterCount: letters.length, activeTasks: tasks.filter((x) => ![3, 4, 'Done', 'Cancelled'].includes(x.Status)).length, openTickets: tickets.filter((x) => !['resolved', 'closed'].includes(x.Status)).length, sentSms: (smsR.data ?? []).filter((x) => [1, 2, 'Sent', 'Delivered'].includes(x.Status)).length, pendingForms: forms.filter((x) => ['manager_pending', 'hr_pending'].includes(x.Status)).length, activeUsers: users.filter((x) => x.IsActive).length },
