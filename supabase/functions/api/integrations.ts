...
async function sms(request: Request, auth: AuthContext, path: string): Promise<Response> {
  ...
  if (request.method === 'PUT' && path === '/sms-settings') { const input = await body<Obj>(request); let uri: URL; try { uri = new URL(String(input.apiUrl)) } catch { throw new HttpError(400, 'آدرس API معتبر نیست') }; if (uri.protocol !== 'https:' || ['localhost', '127.0.0.1'].includes(uri.hostname)) throw new HttpError(400, 'آدرس API باید HTTPS عمومی باشد'); const old = await current(); const values = { ProviderName: input.providerName, ApiUrl: uri.toString(), SenderNumber: input.senderNumber ?? null, Username: input.username ?? null, EncryptedPassword: input.password ? await protect(String(input.password)) : old?.EncryptedPassword ?? '', EncryptedApiKey: input.apiKey ? await protect(String(input.apiKey)) : old?.EncryptedApiKey ?? '', IsActive: Boolean(input.isActive), LetterTemplate: input.letterTemplate, ReferralTemplate: input.referralTemplate, MeetingTemplate: input.meetingTemplate, UpdatedAt: now() }; const r = old ? await db.from('SmsProviderSettings').update(values).eq('TenantId', auth.tenantId).eq('Id', old.Id) : await db.from('SmsProviderSettings').insert({ ...base(auth), ...values }); check(r.error); return json(request, { message: 'تنظیمات پیامک ذخیره شد' }) }
  if (request.method === 'POST' && path === '/sms-settings/test') { const input = await body<Obj>(request); const s = await current(); if (!s?.IsActive) throw new HttpError(400, 'سرویس پیامک فعال نیست'); const token = s.EncryptedApiKey ? await unprotect(s.EncryptedApiKey) : ''; const response = await fetch(s.ApiUrl, { method: 'POST', headers: { 'Content-Type': 'application/json', ...(token ? { Authorization: `Bearer ${token}` } : {}) }, body: JSON.stringify({ to: input.phone, message: input.message, sender: s.SenderNumber, username: s.Username }) }); const log = await db.from('SmsMessages').insert({ ...base(auth), To: input.phone, Body: input.message, Status: response.ok ? 1 : 3, Provider: s.ProviderName, MessageId: null, ErrorMessage: response.ok ? null : (await response.text()).slice(0, 500), SentAt: response.ok ? now() : null, ScheduledAt: null, Cost: null, TemplateId: null, SentByUserId: auth.userId }); check(log.error); if (!response.ok) throw new HttpError(400, 'ارسال پیامک ناموفق بود'); return json(request, { message: 'پیامک آزمایشی ارسال شد' }) }
  throw new HttpError(405, 'عملیات پشتیبانی نمی‌شود')
}
