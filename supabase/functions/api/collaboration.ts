...
async function chat(request: Request, auth: AuthContext, path: string): Promise<Response> {
  ...
  if (request.method === 'POST' && path === '/chat/groups') {
    requirePermission(auth, 'chat.create_group')
  ...
  if (request.method === 'POST' && groupMembers) {
    requirePermission(auth, 'chat.add_member')
  ...
  if (request.method === 'DELETE' && groupMember) {
    requirePermission(auth, 'chat.remove_member')
  ...
  if (request.method === 'POST' && path === '/chat/messages') {
    ...
    const attachmentData = input.attachmentData ? String(input.attachmentData) : null
    if (attachmentData && attachmentData.length > 700_000) throw new HttpError(413, 'حجم فایل پیوست بیش از حد مجاز است (حداکثر حدود ۵۰۰ کیلوبایت)')
    if (String(content).length > 4000) throw new HttpError(413, 'متن پیام بیش از حد طولانی است')
    const row = { ...base(auth), SenderUserId: auth.userId, RecipientUserId: recipientType==='user'?recipientId:null, RecipientContactId:recipientType==='contact'?recipientId:null, Content: content, Kind: kind, AttachmentData: attachmentData, AttachmentName: input.attachmentName ? String(input.attachmentName).slice(0, 260) : null, AttachmentContentType: input.attachmentContentType ? String(input.attachmentContentType).slice(0, 120) : null, AttachmentSize: input.attachmentSize ?? null, VoiceDurationSeconds: input.voiceDurationSeconds ?? null, IsRead: false, ReadAt: null }
...
async function forms(request: Request, auth: AuthContext, path: string, url:URL): Promise<Response> {
  ...
  const editMatch = path.match(/^\/forms\/([0-9a-f-]+)$/i)
  if (editMatch && request.method === 'PATCH') {
    ...
    if (!isPersonnel) {
      requestedHours = requestedLeaveHours(formType, input.data ?? {})
